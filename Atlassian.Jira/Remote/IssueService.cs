...
namespace Atlassian.Jira.Remote
{
    internal class IssueService : IIssueService
    {
        ...
        public async Task<string> GetIssueJsonAsync(string issueKey,
            CancellationToken token = default(CancellationToken))
        {
            var fields = ALL_FIELDS_QUERY_STRING;
            var resource = $"rest/api/2/issue/{issueKey}?fields={fields}";
            var response = await _jira.RestClient.ExecuteRequestAsync(Method.GET, resource, null, token)
        ...
        public async Task<IPagedQueryResult<Issue>> GetIssuesFromJqlAsync(IssueSearchOptions options, CancellationToken token = default(CancellationToken))
        {
            ...
            var result = await _jira.RestClient.ExecuteRequestAsync(Method.POST, "rest/api/2/search/jql", parameters, token).ConfigureAwait(false);
        ...
        public async Task SetOriginalEstimateAsync(Issue issue, string newValue, CancellationToken token = default)
        {
            var resource = $"rest/api/2/issue/{issue.Key.Value}";
            var fields = new JObject();
            var timeTracking = new JObject();
            timeTracking.Add("originalEstimate", JValue.CreateString(newValue));
            fields.Add("timetracking", timeTracking);

            await _jira.RestClient.ExecuteRequestAsync(Method.PUT, resource, new { fields = fields }, token)
                .ConfigureAwait(false);
        }

        public async Task SetTimeRemainingAsync(Issue issue, string newValue, CancellationToken token = default)
        {
            var resource = $"rest/api/2/issue/{issue.Key.Value}";
            var fields = new JObject();
            var timeTracking = new JObject();
            timeTracking.Add("remainingEstimate", JValue.CreateString(newValue));
            fields.Add("timetracking", timeTracking);
