...
namespace Atlassian.Jira
{
    ...
    public class Issue : IRemoteIssueFieldProvider
    {
        ...
        public async Task SetOriginalEstimateAsync(string newValue, CancellationToken token = default)
        {
            if (String.IsNullOrEmpty(_originalIssue.key))
            {
            ...
            await _jira.Issues.SetOriginalEstimateAsync(this, newValue, token).ConfigureAwait(false);
        }

        public async Task SetTimeRemainingAsync(string newValue, CancellationToken token = default)
        {
            if (String.IsNullOrEmpty(_originalIssue.key))
            {
                throw new InvalidOperationException("Unable to link issue, issue has not been created.");
            ...
            await _jira.Issues.SetTimeRemainingAsync(this, newValue, token).ConfigureAwait(false);
        }
